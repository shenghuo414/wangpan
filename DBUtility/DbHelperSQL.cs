...
namespace DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static DataSet Query(string SQLString, int currentSize, int pageSize, params SqlParameter[] cmdParms)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand();
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                {
                    ...
                    try
                    {
                        da.Fill(ds, currentSize, pageSize, "ds");
        ...
        public static DataSet Query(string SQLString, int currentSize, int pageSize)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                DataSet ds = new DataSet();
                try
                {
                    ...
                    command.Fill(ds, currentSize, pageSize, "ds");
                }
        ...
        #region 执行带参数的SQL语句
        public static int ExecuteSqlReturnID(string SQLString,params SqlParameter[] cmdParms)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        public static DataTable RunPagination(string TableName, string PK, string Fields, string WhereStr, string OrderStr, int PageSize, int PageIndex, bool IsCount, out int RecordCount)
        {
            SqlParameter[] Paras =
            {
            ...
            Paras[3].Value = WhereStr;
            Paras[4].Value = OrderStr;
            Paras[5].Value = PageSize;
            Paras[6].Value = PageIndex;
            Paras[7].Value = Convert.ToInt32(IsCount);
            Paras[8].Direction = ParameterDirection.Output;
            DataSet PageSet = RunProcedure("[Pagination]", Paras, "Pager");
            RecordCount = IsCount ? Convert.ToInt32(Paras[8].Value) : 0;
            return PageSet.Tables["Pager"];
        }
